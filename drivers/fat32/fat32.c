...
static void fat_print_table(struct volume_s* vol, u32 sector) {
	...
	kprint("\n");
	kprint("FAT: ");
	kprint_int(sector * 128);
	for (u8 i = 0; i < 128;) {
		u32 curr = fat_load32(vol->buffer + i * 4);
		
		kprint_int((u8)(curr >> 24));
		kprint_int((u8)(curr >> 16));
		kprint_int((u8)(curr >> 8));
		kprint_int((u8)(curr));
		kprint("   ");
		if ((i++ % 4) == 0) {
			kprint("\nFAT: ");
			kprint_int(sector * 128 + i);
...
static fstatus fat_follow_path(struct dir_s* dir, const char* path, u32 length) {
	...
	while (1) {
		...
		if (!fat_dir_search(dir, frag_ptr, frag_size)) {
			kprint_colored("Directory not found\n", 0x0C);
			return 0;
		}
	}	
	return FSTATUS_OK;
...
static void fat_print_info(struct info_s* info) {
	...
	kprint_int(date & 0b11111);
	kprint(" "); 
	kprint_int((date >> 5) & 0b1111);
	kprint(" ");
	kprint(((date >> 9) & 0b1111111) + 1980);
	kprint(" ");
	kprint_int((time >> 11) & 0b11111);
	kprint(" ");
	kprint_int((time >> 5) & 0b11111);
...
void fat32_thread(void* arg) {
	...
	for (u8 i = 0; i < 6; i++) {
		...
		kprint("S: ");
		kprint_int(cluster_size_lut[i].clust_size);
		kprint(" c: ");
		kprint_int(cluster_size_lut[i].sector_cnt);
		kprint("\n");
	}
		...
		if (partitions[i].lba) {
			...
			if (fat_search(mount_buffer)) {
				...
				struct volume_s* vol = (struct volume_s *)
					find_memblock(DRAM_BANK_0, sizeof(struct volume_s));
